...
Beyond the gradual decline of Unity since [their new CEO in 2019, and the myriad of controversies up to his resignation](https://www.nytimes.com/2023/10/09/technology/unity-chief-resigns-after-pricing-backlash.html), I'm more mad that Unity for Linux **STILL CAN'T SCALE THE UI**. Tiny ass 8px font on my high-res monitor means I need to squint when reading, and constantly miss clicking when doing literally anything. Also, older (and arguably better) Unity versions require deprecated packages, and crash whenever I try changing my layout.
Unity scenes are also just one giant file, with the consequence of **endless** Git conflicts when two people *dare* have the *audacity* to work on the same scene in the game engine software.
