...
[my first hackathon in this blogpost.](/blog/first-hackathon)
...
earn a proportional number of coins, which can be spent on expanding and upgrading the garden. Incorporating the
philosophy from established research and other immensely successful idle games, we make progression fun and visceral
through genuine cognitive performance.
...
1. We connected the Muse 2 to our laptop using `brainflow`, a Python library that lets us read raw EEG and
   ...
   - I can easily extend this to more than 2 targets, but for this project we look at whether a person is focused (FO)
     or not (UF), and whether they are fatigured (FA) or not (NF). Hence the four classes are labelled: FO-NF | FO-FA |
     UF-NF | UF-FA
2. Using the packages `pytorch-lightning` and `hydra`, I wrote a fairly standard training script that fits a PyTorch
   ...
   - Pytorch-lightning essentially wraps a normal Pytorch network into its own trainer object, blackboxing the normal
     PyTorch training loop via a Trainer object. Hydra defines a set of YAML config files that can be accessed from main
     and overridden from the console.
   - This `cfg` object is supposed to be passed around as a parameter, but it became really frustrating to deal with
     relative paths and wrong behavior when backend threads needed configs outside a main function. Ended up writing a
...
working with everyone, and I wish yall the best of luck in the future~
